...
const Login: React.FC = () => {
  ...
  const BASE_URL = 'http://localhost:9002/iso/api/v1';

  // Initialize page - equivalent to JSP onload="initializePage()"
  useEffect(() => {
    const downloadStaticCheck = async () => {
      console.log('downloadStaticCheck called');
      try {
        setShowProgressPopup(true);
        
        console.log('Fetching from:', `${BASE_URL}/downloadStatus`);
    ...
    const initializePage = async () => {
      console.log('=== Page Initialized - Login.tsx ===');
      console.log('User logged in:', !!user);
      
      // Always call downloadStaticCheck on page load (matches JSP behavior)
      console.log('Calling downloadStaticCheck on page load...');
    ...
    // Only run initialization if user is not logged in
    if (!user) {
      initializePage();
    } else {
